#!/usr/bin/env node
/**
 * 從證券交易所（TWSE，上市）公開資料抓取，寫成 JSON 檔（web/public/data/）。
 *
 ...
 *
 * 不需要任何環境變數、金鑰或資料庫。資料寫在 web/public/data/，
 * 會隨網站一起部署。儲存層的設計說明見 scripts/jsonStore.js。
 */

import { createStore, canSkip } from './jsonStore.js'
...
const REQUEST_INTERVAL_MS = Number(process.env.TWSE_INTERVAL_MS ?? 3000)

// 以下兩個覆寫只給自動化測試用：把請求導向本機的模擬伺服器、把資料寫到暫存目錄。
// 正常使用時不要設定。
const TWSE_BASE_OVERRIDE = process.env.TWSE_BASE_URL
const DATA_DIR_OVERRIDE = process.env.DATA_DIR
...
async function twseGet(endpoint, params) {
  ...
  if (TWSE_BASE_OVERRIDE) url = url.replace('https://www.twse.com.tw', TWSE_BASE_OVERRIDE)
...
async function addToWatchlist(store, stockIds) {
  ...
  // 與既有清單合併，已經在清單裡的不重複加入
  const current = store.readWatchlist()
  const known = new Set(current.map((s) => s.stock_id))
  const added = found.filter((s) => !known.has(s.stock_id))
  const already = found.filter((s) => known.has(s.stock_id))

  store.writeWatchlist([...current, ...added])
  for (const s of found) store.setStockInfo(s.stock_id, s)
  store.flush()

  for (const s of added) console.log(`   ✅ ${s.stock_id} ${s.name}`)
  for (const s of already) console.log(`   ・ ${s.stock_id} ${s.name}（原本就在清單裡）`)
  console.log(`\n追蹤清單現在共 ${current.length + added.length} 檔，存在 ${store.paths.watchlist}`)
...
async function fetchRange(store, { startDate, endDate }) {
  ...
  // 先算出這次真正要打證交所的有幾次，才能給出準確的預估時間
  let pending = 0
  for (const day of days) {
    for (const spec of DATASETS) {
      if (!canSkip(store.getLogEntry(spec.name, day), day, hash, today)) pending++
    }
  }
  ...
  console.log(`   其中 ${totalTasks - pending} 次可以跳過（先前已用同樣的追蹤池抓過，或已確認是假日）`)
  console.log(`   這次要抓 ${pending} 次，節流 ${REQUEST_INTERVAL_MS}ms／次，預估 ${Math.ceil(pending * REQUEST_INTERVAL_MS / 60000)} 分鐘`)
  console.log(`   資料寫在 ${store.dataDir}\n`)

  let ok = 0, skipped = 0, failed = 0

  for (const day of days) {
    for (const spec of DATASETS) {
      if (canSkip(store.getLogEntry(spec.name, day), day, hash, today)) { skipped++; continue }
      ...
      try {
        ...
        store.setLogEntry(spec.name, day, {
          status: 'failed', row_count: 0, message: err.message.slice(0, 500), watchlist_hash: hash
        })
        failed++
        continue
      }
      ...
      if (!isOk(json)) {
        store.setLogEntry(spec.name, day, {
          status: 'nodata', row_count: 0, message: String(json?.stat ?? '').slice(0, 500), watchlist_hash: hash
        })
        skipped++
        continue
      }
      ...
      let written = 0
      for (const row of mine) written += store.upsertRows(row.stock_id, spec.kind, [row])
    ...
    // 每處理完一天就寫回磁碟，回補途中被中斷最多只損失一天
    store.flush()
  }

  console.log(`\n完成。成功 ${ok}、跳過 ${skipped}、失敗 ${failed}。`)
  if (failed > 0) console.log('失敗的日期已記錄在 fetch_log.json，再跑一次就會自動重試。')
}
...
async function main() {
  ...
  if (args.check) {
    ...
    const store = createStore(DATA_DIR_OVERRIDE)
    const watchlist = store.readWatchlist()
    console.log(`\n📁 資料目錄：${store.dataDir}`)
    console.log(`   追蹤清單目前 ${watchlist.length} 檔` +
      (watchlist.length ? `：${watchlist.map((w) => `${w.stock_id} ${w.name ?? ''}`.trim()).join('、')}` : ''))
    return
  }

  const store = createStore(DATA_DIR_OVERRIDE)

  if (args.add) return addToWatchlist(store, args.add)

  const today = isoDate(new Date())
  if (args.mode === 'backfill') {
    const start = new Date()
    start.setUTCFullYear(start.getUTCFullYear() - args.years)
    await fetchRange(store, { startDate: isoDate(start), endDate: today })
